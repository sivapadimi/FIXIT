    ...
    <script>
        ...
        function runCode(problemId) {
            ...
            setTimeout(() => {
                ...
                problem.test_cases.forEach((testCase, index) => {
                    if (!testCase.hidden) {
                        total++;
                        // Check if the bug is fixed: removed "+ 1" from range and fixed array bounds check
                        const isCorrect = (code.includes('range(len(arr))') && !code.includes('range(len(arr) + 1')) || 
                                         (code.includes('for i in range(len(arr))') && !code.includes('for i in range(len(arr) + 1'));
        ...
        function loadUserFromStorage() {
            ...
            if (stored) {
                ...
                // Update UI
                document.querySelectorAll('.nav-link')[3].outerHTML = `
                    <span style="color: #f9fafb; margin-right: 1rem;">
                        <i class="fas fa-user"></i> ${currentUser.username}
                    </span>
                    <button class="btn btn-secondary" onclick="logout()">Logout</button>
