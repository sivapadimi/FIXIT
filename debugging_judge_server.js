...
function createSampleProblems() {
    ...
    const problem2 = new ProblemDefinition(
        2,
        "Array Bounds - Fix Loop Bug",
        "Fix array bounds issue in loop",
        [
            new BugDefinition(
                "loop_bounds_bug",
...
function createServer() {
    ...
    // Load problems from config
    judge.loadProblemsFromConfig('./problems_config.json');
    
    app.use(express.json());
    
    // API endpoint for submission validation
    ...
    app.get('/api/problems', (req, res) => {
        const problems = Array.from(judge.problems.values()).map(problem => ({
            problemId: problem.problemId,
            title: problem.title,
            description: problem.description,
            language: problem.language,
            totalPoints: problem.totalPoints,
            bugs: problem.bugs.map(bug => ({
                bugId: bug.bugId,
    ...
    app.listen(PORT, () => {
        console.log(`FixIt Judge Server running on port ${PORT}`);
        console.log(`API endpoints:`);
        console.log(`  POST /api/validate - Validate submission`);
